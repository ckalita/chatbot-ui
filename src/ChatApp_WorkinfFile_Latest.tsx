...
export default function ChatApp() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:9090/ai/RAG/chatBotUsingRag?question=${encodeURIComponent(
          input
        )}`,
        { method: "POST" }
      );
  ...
  const generateImage = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:9090/ai/image/getImage?question=${encodeURIComponent(
          input
        )}`
      );
  ...
  const getImageDescription = async () => {
    ...
    try {
      const res = await fetch("http://localhost:9090/ai/image/getImageDescription", {
        method: "POST",
        body: formData,
      });

      const description = await res.text();
      console.log("image description is :", description);
  ...
  const textToAudio = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:9090/ai/speech/textToAudio?question=${encodeURIComponent(
          input
        )}`
      );
      const blob = await res.blob();
  ...
  const audioToText = async () => {
    ...
    try {
      const res = await fetch("http://localhost:9090/ai/speech/getAudioToText", {
        method: "POST",
        body: formData,
      });
